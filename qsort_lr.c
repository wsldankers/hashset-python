...
void qsort_lr(void *const pbase, size_t total_elems, size_t size,
	...
	if(total_elems > MAX_THRESH) {
		...
		while(STACK_NOT_EMPTY) {
			...
			if(cmp((void *)mid, (void *)lo, size, arg) < 0)
				memswap(mid, lo, size);
			if(cmp((void *)hi, (void *)mid, size, arg) < 0)
				memswap(mid, hi, size);
			else
				goto jump_over;
			if(cmp((void *)mid, (void *)lo, size, arg) < 0)
			...
			do {
				while(cmp((void *)left_ptr, (void *)mid, size, arg) < 0)
					left_ptr += size;

				while(cmp((void *)mid, (void *)right_ptr, size, arg) < 0)
					right_ptr -= size;

				if(left_ptr < right_ptr) {
					memswap(left_ptr, right_ptr, size);
					if(mid == left_ptr)
						mid = right_ptr;
...

	{
		...
		for(run_ptr = tmp_ptr + size; run_ptr <= thresh; run_ptr += size)
			if(cmp((void *)run_ptr, (void *)tmp_ptr, size, arg) < 0)
				tmp_ptr = run_ptr;

		if(tmp_ptr != base_ptr)
			memswap(tmp_ptr, base_ptr, size);
		...
		while((run_ptr += size) <= end_ptr) {
			...
			while(cmp((void *)run_ptr, (void *)tmp_ptr, size, arg) < 0)
				tmp_ptr -= size;
